...
* （推荐）已实施服务器端插入功能。
   ...
   * 搜索、社交和商务客户：
      ...
      * 对于其他广告网络，或 [!DNL Google Ads] 和 [!DNL Microsoft® Advertising] 帐户具有 [!UICONTROL Auto Upload] 设置已禁用，手动 [将参数添加到帐户级别的附加参数](/help/search-social-commerce/campaign-management/accounts/ad-network-account-manage.md){target="_blank"}，以将其附加到基本URL中。
...
* 未实现服务器端插入功能：
   ...
   * 搜索、社交和商务客户：
      ...
      * 对于所有其他广告网络上的广告，请手动 [将AMO ID参数添加到帐户级别的附加参数](/help/search-social-commerce/campaign-management/accounts/ad-network-account-manage.md){target="_blank"}，以将其附加到基本URL中。
