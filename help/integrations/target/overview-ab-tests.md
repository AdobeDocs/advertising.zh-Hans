---
title: 在Adobe Target中为Adobe广告广告配置A/B测试
description: 了解如何在中设置A/B测试 [!DNL Target] 适用于您的DSP和 [!DNL Search] 广告。
...
# 在Adobe Target中为Advertising DSP和配置A/B测试 [!DNL Advertising Search] 广告
...
Adobe广告和Adobe Target使营销人员更容易通过付费媒体和现场消息传送提供个性化的互联体验。 通过在产品之间共享信号，您可以：
...
1. 从DSP展示数据配置Audience Manager区段：
   ...
   1. 输入 **键** 和 **值** 用于确定在哪个级别分组区段用户的信号。 使用 [支持的键](https://experienceleague.adobe.com/docs/audience-manager/user-guide/implementation-integration-guides/media-data-integration/impression-data-pixels.html?lang=en) 其值与添加到Audience Manager展示事件像素的宏相对应。
...
* 在工作区中创建特定于运行测试的Adobe广告营销活动、包或版面的面板。 使用摘要可视化图表显示同一报表中的Adobe广告量度和Target测试性能。

* 优先使用现场量度（例如访问和转化）来衡量绩效。

* 了解Adobe广告中的汇总媒体量度（例如展示次数、点击量和成本）无法与Target量度匹配。
...
[AdobeDebugger中的补充数据ID (SDID)值](/help/integrations/assets/target-troubleshooting-sdid.png)

* 在同一登陆页面上，验证a) “解决方案> Target”下的“Adobe调试器”中显示的主机名是否与b)中显示的“跟踪服务器”相匹配 [!DNL Target] （在“目标和设置”>“报表设置”下）。

   [!DNL Analytics For Target] 需要 [!DNL Analytics] 要以调用形式发送的跟踪服务器 [!DNL Target] 到 [!DNL Modstats] 数据收集服务器。<!-- just "to Analytics?"-->

[AdobeDebugger中的主机名值](/help/integrations/assets/target-troubleshooting-hostname.png)
