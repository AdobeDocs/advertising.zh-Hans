...
| 嵌入代码缺少或环境错误 | 验证网页上的[[!DNL Tags] 嵌入代码](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/environments/environments)是否引用了正确的环境(Dev/Stage/Prod)。 在`<head>`标记中查找`//assets.adobedtm.com/...`脚本标记的环境。 |
| 异步与同步加载冲突 | 确保每个网页仅存在一个[[!DNL Tags] 嵌入代码](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/environments/environments)。 重复嵌入代码会导致争用情况。 |
| 内容安全策略(CSP)阻止 | 将`edge.adobedc.net` `and assets.adobedtm.com`添加到您的[CSP `connect-src`和`script-src`指令](https://experienceleague.adobe.com/en/docs/experience-platform/collection/use-cases/configuring-a-csp)。 |
...
* 规则已保存并包含在活动[库内部版本](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/builds)中。
* 事件类型与实际页面行为（例如，[!UICONTROL Library Loaded]与[!UICONTROL DOM Ready]对比[!UICONTROL Window Loaded]）匹配。
* 规则的条件限制不太严格。 通过临时[删除条件](https://experienceleague.adobe.com/en/docs/experience-platform/tags/ui/rules)来测试，以隔离问题。
* 规则顺序正确。 如果多个规则共享同一事件，请检查[规则排序](https://experienceleague.adobe.com/en/docs/experience-platform/tags/ui/rules)。
...
| 原因 | 修复 |
| ----- | --- |
| 更改未添加到库。 | 在发布工作流程中，确认您的更改已添加到开发环境中的[库](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/libraries)。 如果您[对库](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/libraries#manage-library-changes)进行更改，请添加资源，然后保存并生成库。 |
...
| 架构中缺少必需的Adobe Advertising字段。 | 确保`_experience.adcloud.conversionDetails`下的[架构](https://experienceleague.adobe.com/en/docs/platform-learn/implement-web-sdk/initial-configuration/configure-schemas)中存在必需的Adobe Advertising字段。 请参阅“[引用：必需的架构字段](#required-schema-fields)”。<br><br>如果缺少任一字段，请确认&#x200B;**Adobe Advertising Cloud ExperienceEvent完整扩展**&#x200B;字段组已保存到架构中，然后刷新架构编辑器。 |
