...
* 广告商必须实施[the [!DNL Adobe CX Enterprise Identity (ECID) Service]](https://experienceleague.adobe.com/zh-hans/docs/id-service/using/intro/overview)版本2.0或更高版本。 [!DNL Identity Service]提供了一个通用的永久性ID，用于在CX Enterprise的所有解决方案中标识您的访客。

  实施包括将[!DNL Identity service]代码添加到广告商网站上的每个网页。

* 组织必须为CX Enterprise服务[&#128279;](https://experienceleague.adobe.com/zh-hans/docs/core-services/interface/services/overview)启用，并且具有CX Enterprise [!DNL Organization ID]（以前称为[!DNL IMS org ID]）。

  [!UICONTROL Organization ID]允许拥有多个Adobe CX Enterprise产品的组织在某些产品之间共享数据。
...
* 将两个目标映射到所有Audience Manager区段，从而允许Audience Manager与DSP广告商帐户（该帐户与用于Audience Manager的相同CX Enterprise [!DNL Organization ID]关联）共享这些区段。
...
1. 在[!DNL Search, Social, & Commerce]内，[!DNL Search, Social, & Commerce]实施团队或具有直接访问客户端管理器角色的用户应从[!UICONTROL Search, Social, & Commerce] > [!UICONTROL Admin] > [!UICONTROL Audience Manager Setup]启动导入。
   ...
   需要组织的Adobe CX Enterprise [!DNL Organization ID] ([!DNL IMS org ID])。 ID必须与用于组织的Audience Manager帐户的ID相同。
