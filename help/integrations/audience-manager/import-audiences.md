...
要在DSP或[!DNL Adobe]中访问[!DNL Creative]受众，必须将受众导入DSP。 要访问[!DNL Adobe]中的[!DNL Search, Social, & Commerce]受众，必须将受众导入到[!DNL Search, Social, & Commerce]中。
...
* 组织必须为Experience Cloud服务[启用](https://experienceleague.adobe.com/zh-hans/docs/core-services/interface/services/overview)，并且具有Experience Cloud [!DNL Organization ID]（以前称为[!DNL IMS org ID]）。

  [!UICONTROL Organization ID]允许拥有多个Adobe Experience Cloud产品的组织在某些产品之间共享数据。

* （使用[!DNL Analytics]的广告商）该广告商必须[使用 [!DNL Analytics] `appMeasurement.js`版本1.6.4或更高版本实施](https://experienceleague.adobe.com/zh-hans/docs/analytics/implementation/js/overview)。
...
* [位置设置](/help/dsp/campaign-management/placements/placement-settings.md#audience-targeting)：在[!UICONTROL Adobe Segments]分区的[!UICONTROL Audience Targeting]选项卡上。
...
在[!DNL Search, Social, & Commerce]中，当您从[!DNL Google] > [!UICONTROL Data Source] > [!UICONTROL Adobe Audience]使用[!UICONTROL Campaigns]“[!UICONTROL Audiences]”创建[!UICONTROL Library]受众时，区段可用。
