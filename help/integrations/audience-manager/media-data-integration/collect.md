---
title: 从Advertising DSP营销活动中收集点击和展示数据
...
* `${TM_CAMPAIGN_ID_NUM}`是DSP中的数字营销活动ID。 如果要对单个促销活动ID进行硬编码，而不是使用DSP宏，请在促销活动设置中找到该ID。
...
实施事件标记后，数据会流入Audience Manager数据收集服务器。 请先完成以下任务，然后才能在报表中使用数据。
