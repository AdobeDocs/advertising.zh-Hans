...
* 除了由Cookie或设备ID跟踪的用户之外，某些供应商提供的第三方区段还可以自动包含通用ID。 例如，以下项的区段： [!DNL Eyeota] 可能会自动包含ID5 ID，以及以下项的区段： [!DNL Lotame] 可能包括UID2.0 ID。 区段详细信息包括每种类型的大小。 每个区段通常收取的使用费（在区段名称旁边列出）将适用；ID5 ID无需额外付费。
