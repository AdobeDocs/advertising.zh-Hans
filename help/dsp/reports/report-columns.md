...
| [!UICONTROL Dimension] | [!UICONTROL Date/Time] | 天 [!UICONTROL of Week] | 特定日期，如 [!UICONTROL Monday] 或 [!UICONTROL Tuesday]. |
...
| [!UICONTROL Dimension] | [!UICONTROL Placement] | [!UICONTROL Device Environment] | 放置所针对的设备环境： (*[!UICONTROL Desktop]*， *[!UICONTROL Mobile]*，和/或 *[!UICONTROL Connected TV])*. |
...
| [!UICONTROL Metric] | [!UICONTROL Frequency] | [!UICONTROL Estimated Impressions (Device/Browser)] | (包含在 [!UICONTROL Frequency (by Impression)] 报表)给定频率划分的预计展示次数。 DSP的估计基于一组展示次数。 对于某些库存，发布者不会传递设备标识符，并且这些展示不会包含在此值中。 中有一个类似的量度 [!UICONTROL Frequency (by App/Site)] 报表，但并未估计该量度。 |
| [!UICONTROL Metric] | [!UICONTROL Frequency] | [!UICONTROL Estimated Uniques (Device/Browser)] | (包含在 [!UICONTROL Frequency (by Impression)] 报表)在给定频率下记录的独特浏览器或设备数。 DSP的估计基于一组展示次数。 对于某些库存，请勿传递设备标识符，并且这些展示不会包含在此值中。 中有一个类似的量度 [!UICONTROL Frequency (by App/Site)] 报表，但并未估计该量度。 |
...
| [!UICONTROL Metric] | [!UICONTROL Household] | [!UICONTROL Frequency Overlap] | 仅按报告的维度到达住户的频率，包括维度最多三个值的交集。 例如，如果您使用 [!UICONTROL Placement] 然后，您可以查看单个版面所达到的频率、任意两个版面组合所达到的频率以及任意三个版面组合所达到的频率。 |
| [!UICONTROL Metric] | [!UICONTROL Household] | [!UICONTROL Incremental Household Reached] | 仅通过报告维度实现的家庭数量，计算方式为 <code>[仅报告维度到达的IP地址] - [任何其他维度访问的IP地址]</code>. |
| [!UICONTROL Metric] | [!UICONTROL Household] | [!UICONTROL % Incremental Household Reached] | 仅通过报告维度实现的家庭百分比，计算方式为 <code>[维度访问的IP地址的百分比] - [任何其他维度访问的IP地址百分比]</code>. |
...
| [!UICONTROL Metric] | [!UICONTROL Household] | [!UICONTROL Measurable Impressions (Overlap)] | 仅报告维度提供的可测量展示总数，包括最多三个维度值的交集。 例如，如果您使用 [!UICONTROL Placement] ”维度中，然后您可以看到单个版面所达到的可测量展示次数、任意两个版面组合所达到的可测量展示次数，以及任意三个版面组合所达到的可测量展示次数。 |
...
| [!UICONTROL Metric] | [!UICONTROL Household] | [!UICONTROL Unique Household (Overlap)] | 仅报告维度实现的唯一家庭总数（不同的IP地址），包括维度中最多三个值的交集。 例如，如果您使用 [!UICONTROL Placement] 然后，您可以看到由单个投放位置到达的唯一家庭、由任意两个投放位置组合到达的共同家庭，以及由任意三个投放位置组合到达的共同家庭。 |
...
| [!UICONTROL Metric] | [!UICONTROL Household Conversions] | [!UICONTROL Incremental Household Reached] | 仅通过报告维度实现的家庭数量，计算方式为 [仅报告维度到达的IP地址] - [任何其他维度访问的IP地址]. |
| [!UICONTROL Metric] | [!UICONTROL Household Conversions] | [!UICONTROL % Incremental Household Reached] | 仅通过报告维度实现的家庭百分比，计算方式为 [维度访问的IP地址的百分比] - [任何其他维度访问的IP地址百分比]. |
...
| [!UICONTROL Metric] | [!UICONTROL Performance] | [!UICONTROL Gross CPA] | 每次购买的平均总成本，计算方式为 <code>[!UICONTROL Gross Spend] / [!UICONTROL Custom Goal]</code>. |
...
| [!UICONTROL Metric] | [!UICONTROL Performance] | [!UICONTROL Net CPM] | 每1000次展示的平均净成本，计算方法为 <code>[!UICONTROL Net Spend] / [!UICONTROL Impressions] x 1000</code>. |
...
| [!UICONTROL Metric] | [!UICONTROL Performance] | [!UICONTROL Total Other eCPM] | 其他费用每1000次展示的平均净成本，计算方式为 <code>[!UICONTROL Net Spend (Other)] / [!UICONTROL Impressions] x 1000</code>. |
| [!UICONTROL Metric] | [!UICONTROL Performance] | [!UICONTROL % bid at Max CPM] | 在最大CPM上出价的总出价百分比。 |
| [!UICONTROL Metric] | [!UICONTROL Performance] | [!UICONTROL Unique Users Bid On] | DSP为其竞价投放的不同用户的数量。 |
...
| [!UICONTROL Metric] | [!UICONTROL Spend] | [!UICONTROL Estimated Tax on Media] | 媒体上的估计税费，包括应用于DSP中的媒体成本再计费和技术费用服务的税费。 |
...
| [!UICONTROL Metric] | [!UICONTROL Viewability] | [!UICONTROL Viewability Rate (%)] | 所有可衡量的展示次数中的可见展示次数的百分比，计算方式为 <code>[!UICONTROL Viewable Impressions] / [!UICONTROL Measurable Impressions]</code>. |
| [!UICONTROL Metric] | [!UICONTROL Viewability] | [!UICONTROL Viewable Impressions] | 被视为可查看的广告展示次数。 |
| [!UICONTROL Conversion Metrics] | [在报告设置中按广告商分组] | [特定于广告商的转化] | 指定的特定于广告商的转化量度或Adobe Analytics事件的总数。 |
| [!UICONTROL Custom Goals] | [在报告设置中按广告商分组] | [特定于广告商的自定义目标] | 指定中包含的所有转化的加权总和 [自定义目标](/help/dsp/optimization/custom-goal.md). |
