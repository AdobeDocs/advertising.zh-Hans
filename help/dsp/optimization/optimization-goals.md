...
| 优化目标 | 描述 | 何时使用此目标 |
| -----------| ---------- | ---------- |
| [!UICONTROL Always Max Bid & Clickthrough Rate] | 通过包级别优化，预算分配可优先处理点击率最高的投放位置。<br><br>拍卖评估将优先处理CTR，以实现支出目标。 提交的竞价始终为所设置的[!UICONTROL Max Bid]，但如果投放位置花费良好，则预测的CTR阈值将变得更加严格。 | 促销活动类型：品牌推广<br><br>基准：最高点进率<br><br>广告类型：前置广告，显示<br><br><b>注意：</b>如果您有不需要超越的固定CPM目标和必须最大化的CTR目标，请使用此目标。 将最高出价设置为所需的CPM目标，DSP在尝试花费全部预算的同时实现最佳的CTR。 |
