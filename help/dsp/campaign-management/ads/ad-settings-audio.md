...
>[!TIP]
>
> 在中，使用将广告附加到投放位置时容易找到的名称 [!UICONTROL Ads] 视图，以及在报表中。 例如，描述设备类型和某些关键属性（例如Holiday Product Preview： 30sec Audio”）。
...
**[!UICONTROL Final VAST Tag]：** （仅使用VAST标记的广告）第三方广告源的URL，带有必要的 [Advertising DSP跟踪宏](/help/dsp/campaign-management/macros.md) 已插入（如果适用）。

**[!UICONTROL Select Rate]：** （仅限具有权限的用户）通过Adobe计费的预协商费率，或者您已经协商并通过供应商计费的费率之一。 要添加费率，请联系您的Adobe客户团队。
