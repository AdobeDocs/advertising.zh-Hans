...
您可以在任意位置手动插入宏，例如在VAST标记中、在任何URL中，或者在DSP或第三方事件像素中。 但是，每个DSP客户端和合作伙伴都具有不同的广告标记格式，并且宏应相应地插入到标记中的不同位置。 每次与新客户或合作伙伴合作时，请向他们索取相关文档，了解如何在其广告标记中插入DSP所流量的宏。
...
要使用第三方显示标记准确跟踪广告的点击次数，DSP需要显示点击宏。 只需要宏的一个版本；相关的宏取决于标记类型。
...
在执行以下操作时，DSP会在第三方显示标签中自动插入显示单击宏：
