...
我们的欺诈监视团队与业界领先的合作伙伴（如[!DNL Interactive Advertising Bureau]、[!DNL Trust and Accountability Group] [!DNL (TAG)]和[!DNL WhiteOps]）紧密合作，仔细管理我们平台上的库存。 通过主动管理我们的供应，DSP确保跨平台的所有广告商都免受非人为流量（机器人、爬虫、数据中心流量和欺诈）的影响，并仅在品牌安全的环境中投放。
...
表示[[!DNL Ads.txt] [!DNL Authorized Digital Sellers]的](https://iabtechlab.com/ads-txt)是[!DNL Interactive Advertising Bureau] ([!DNL IAB])在2017年6月发起的一项计划，旨在促进在公开市场上正确呈现库存，从而打击非法流量来源和域欺骗。 参与的出版商和发行商通过将`ads.txt`页面保留在域的顶层（例如`example.com/ads.txt`），公开声明有权销售其数字库存的公司，以及这些关系的性质。

DSP通过读取每个发布者的[!DNL ads.txt]文件并允许您选择仅向已验证的`ads.txt`销售者购买来支持[!DNL ads.txt]。 例如，通过将我们看到的访问`nytimes.com`的卖家与《纽约时报》的`ads.txt`文件相匹配，我们可以识别哪些是合法的，哪些是非法的，如果投放位置配置为仅从经过验证的卖家处购买，我们将阻止违法者。<!-- can we actually mention NY Times? -->

您可以为每个广告商[!DNL ads.txt]设置默认的<!-- [default ads.txt controls for each advertiser](/help/dsp/admin/advertiser-settings.md) -->控件，然后可选[将每个版面的设置](/help/dsp/campaign-management/placements/placement-settings.md)自定义为：
...
此外，Adobe还与[!DNL IAB]和[!DNL TAG]密切合作，确保通过行业标准的强大欺诈阻止功能来保护我们的广告商，并利用诸如[!DNL ads.txt]（请参阅上一部分）、[!DNL IAB]机器人和蜘蛛程序列表以及[!DNL TAG]数据中心IP列表之类的工具。

我们的团队通过多维度质量方法监控异常和无效流量模式，确保受保护库存上的无效流量低于3%。 任何可疑的库存 — 包括特定域上的库存或者来自特定出版商或卖家的库存 — 都会立即在整个平台上被阻止。

### 库存映射、分层和分类

清单映射是在将新清单添加到我们的平台之前，对所有新清单所需的详细审查和载入流程。 此过程旨在确保DSP上所有库存的安全性和质量。
...
* **分层：**&#x200B;我们全面检查整个生态系统中的品牌存在感，以便跨不同层对库存进行分类。 您可以[将投放位置](/help/dsp/campaign-management/placements/placement-settings.md)定位到这些层以获得所需的覆盖级别：
...
DSP提供了全局阻止站点列表，以及创建广告商和帐户的自定义阻止站点列表的选项。
...
>[!NOTE]
>
>您可以通过启用[!UICONTROL Allow unscreened sites]投放位置设置[中的“](/help/dsp/campaign-management/placements/placement-settings.md)”选项，选择绕过附加到受信任私人交易的标准显示广告的全局阻止站点列表。 如有必要，Adobe客户团队还可以选择在交易的发布者设置中为公开（拍卖级别）交易禁用网站阻止。
...
与[!DNL Adobe's]的[!DNL Adelaide]合作伙伴关系为广告商提供了对Adelaide量度“[!DNL Attention Units]”的支持，该量度基于眼睛跟踪、曝光和结果数据来测量媒体质量。

[位置级别竞价前注意目标定位](/help/dsp/campaign-management/placements/placement-settings.md)允许广告商定位特定注意级别以提高客户参与度。

此外，广告商可以为任何促销活动启用投放级别[量度[!UICONTROL Attention Score]（各展示次数中](/help/dsp/campaign-management/campaigns/campaign-settings.md#attention-measurement)的加权平均数）的[!DNL Attention Units]跟踪，以了解哪种投放策略产生的业务效果最佳。
...
主题定位要求您直接使用合作伙伴平台创建自定义主题区段。 创建区段后，您可以在[部分中为每个投放位置[!UICONTROL Audience Targeting]定位或排除区段ID &#x200B;](/help/dsp/campaign-management/placements/placement-settings.md)。 此功能可能需要额外付费。

若要创建[!DNL Comscore]帐户和自定义主题区段，您可以在[!DNL Activation Segment Manager]https://agents.comscore.com[上请求](https://agents.comscore.com)的登录。 有关设置自定义区段的完整说明，请参阅[[!DNL Comscore] 帮助中心](https://comscoreactivation.zendesk.com/hc/)。 创建自定义区段时，[!DNL Segment Manager]中会显示这些区段的费用。
...
您可以为每个广告商[!DNL DoubleVerify]指定一个<!-- [specify a DoubleVerify segment ID for each advertiser](/help/dsp/admin/advertiser-settings.md) -->区段ID，然后可选[为每个投放位置[!UICONTROL Authentic Brand Safety]启用或禁用](/help/dsp/campaign-management/placements/placement-settings.md)。 DSP按区段ID的使用情况向帐户收费。

有关功能的更多信息，请直接联系[!DNL DoubleVerify]，或联系您的Adobe客户团队。
