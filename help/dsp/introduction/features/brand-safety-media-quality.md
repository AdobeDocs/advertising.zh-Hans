...
>[!NOTE]
>
>您可以通过启用[投放位置设置](/help/dsp/campaign-management/placements/placement-settings.md)中的“[!UICONTROL Allow unscreened sites]”选项，选择绕过附加到受信任私人交易的标准显示广告的全局阻止站点列表。 如有必要，Adobe客户团队还可以选择在交易的发布者设置中为公开（拍卖级别）交易禁用网站阻止。
...
DSP已与[!DNL DoubleVerify]合作提供其[!DNL Authentic Brand Suitability]定位解决方案，该解决方案允许您创建一组集中的品牌适用性要求，以在所有购买平台中定位以实现一致性。

构建具有必要定向的[!DNL DoubleVerify]区段后，您可以在DSP中使用它来跨Web环境复制具有预竞价的竞价后块规则。

您可以为每个广告商指定一个[!DNL DoubleVerify]区段ID，然后可选[为每个投放位置](/help/dsp/campaign-management/placements/placement-settings.md)启用或禁用[!UICONTROL Authentic Brand Suitability]。 DSP按区段ID的使用情况向帐户收费。
