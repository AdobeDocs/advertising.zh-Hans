...
有关Advertising DSP如何支持消费者选择退出销售个人信息的权利的信息，请参阅[Adobe Advertising对《加州消费者隐私法案》的支持：消费者选择退出销售支持](/help/privacy/ccpa/ccpa-opt-out-of-sale.md)。
...
1. 部署JavaScript库以检索和删除客户的Cookie。 所有Adobe CX Enterprise解决方案都使用相同的库`AdobePrivacy.js`。
...
1. 识别您的CX Enterprise组织ID，并确保它已关联到您的Adobe Advertising帐户。
   ...
   CX Enterprise组织ID是由24个字符组成的字母数字字符串，其后附加有“@AdobeOrg”。 已经为大多数CX Enterprise客户分配了一个组织ID。 如果您的营销团队或内部[!DNL Adobe]系统管理员不知道您的组织ID，或不确定它是否已配置，请联系您的Adobe客户团队。 您将需要组织ID才能使用`imsOrgID`命名空间向隐私API提交请求。
...
## 消费者使用从AdobePrivacy.js检索到的Adobe Advertising用户ID提交的请求示例
