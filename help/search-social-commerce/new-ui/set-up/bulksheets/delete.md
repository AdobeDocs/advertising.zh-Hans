---
title: （新UI）删除上传的批量工作表和错误文件
description: 了解如何在新的Search、Social和Commerce UI中删除批量处理工作表文件和错误文件。
feature: Search Bulksheets
product_v2:
  - id: a829a185-511f-4bf8-8dcf-9e684f8011cf
feature_v2:
  - id: aed5e38a-3e62-42fa-8d16-cd080729b2a0
subfeature_v2:
  - id: e58024d1-d6da-420c-80af-6be211808316
  - id: f3d33161-c519-436e-bbbd-730ba428736b
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
source-git-commit: 739034010787c2016720bef37fb75dc8efbae58b
workflow-type: tm+mt
source-wordcount: 147
ht-degree: 0%

---

# （新UI）删除上传的批量工作表和错误文件

您可以手动删除批量处理工作表文件、登陆页验证错误文件和其他错误文件。 这些文件在上传或生成后30天自动删除。

>[!TIP]
>
>如果您发布了批量工作表但收到了错误，则在解决所有错误之前不要删除错误文件。

1. 在主菜单中，单击&#x200B;**[!UICONTROL Setup]** \> **[!UICONTROL Bulksheets]**。

1. 选中要删除的每个文件旁边的复选框。

1. 在批量操作工具栏中，单击&#x200B;**[!UICONTROL Delete]**。

1. 在确认消息中，单击&#x200B;**[!UICONTROL Confirm]**&#x200B;以确认删除。

   如果计划发布任何选定的批量工作表，则在确认之前会通知您。 删除计划的批量工作表会取消待处理帖子。

>[!MORELIKETHIS]
>
>* [（新UI）关于使用批量处理工作表管理营销活动数据](about.md)
>* [（新UI）停止正在进行的批量处理工作表作业](stop-job.md)
