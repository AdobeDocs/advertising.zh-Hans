...
### 在何处查看有关整个UI中的约束的信息<!-- wording? -->

除了打开[[!UICONTROL Constraints]视图](#constraints-view)之外，您还可以通过以下方式查看与您的约束相关的信息：

* 所有约束都是名为“[”的单个](https://experienceleague.adobe.com/docs/advertising/search-social-commerce/campaign-management/label-classifications/classification-about.html?lang=zh-Hans)标签分类[!UICONTROL Constraints]的标签值。

   * “[!UICONTROL Constraints]”包含在默认和自定义视图设置以及计划报告的“[!UICONTROL Classifications]”列表中。 可随处添加列，以查看分配给相关实体的约束。

   * 当您下载批量处理工作表时，“[!UICONTROL Constraints]”在[!UICONTROL Classifications]”对话框中适用实体的“[!UICONTROL Download Bulksheet]”列下列出。 当包含列时，下载的批量工作表将包含分配给相关图元的任何约束。
...
您可以从[!UICONTROL Constraints]视图创建、编辑和更改约束的状态。

>[!NOTE]
>
>您可以从相关的实体管理视图（例如，促销活动级别约束的[!UICONTROL Campaigns]视图）将约束分配给竞价单位，也可以取消对它们的分配。
...
| | [!UICONTROL Set constraint options for Impression Share] | （仅限[!UICONTROL Impression Share]个约束）设置包括：<ul><li>**[!UICONTROL Min Bid]** （可选）相关竞价单位的最低基本竞价。</li><li>**[!UICONTROL Max Bid]：**（可选）相关竞价单位的最大基本竞价。</li><li>**[!UICONTROL Min Impression Share]：**&#x200B;最低展示份额（百分比）将触发适用竞价单位的约束。 必须介于10和90之间。 **注意：**&#x200B;当约束不具成本效益时，优化功能可能会覆盖它。</li><li>**[!UICONTROL Max Impression Share]：**&#x200B;最高展示份额（百分比）将触发适用竞价单位的约束。 必须介于10和90之间。**注意：**&#x200B;当约束不具成本效益时，优化功能可能会覆盖它。</li></ul>> |
...
| | [!UICONTROL Data Evaluation Period] | （设置条件时）为指定条件评估数据的时段。 如果选择&#x200B;*[!UICONTROL Custom date range]，*，请通过以&#x200B;**[!UICONTROL Start Date]**&#x200B;格式输入每个日期（如2026年3月29日的03-29-2026）或单击&#x200B;**[!UICONTROL End Date]**&#x200B;日历按钮`MM-DD-YYYY`日历按钮![打开日历并选择每个日期来指定](/help/search-social-commerce/assets/calendar-new.png "和")。 |
