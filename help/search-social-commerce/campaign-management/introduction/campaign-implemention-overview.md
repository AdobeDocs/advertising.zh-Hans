...
在激活营销活动并选择性地将其添加到产品组合后，Adobe客户管理团队、机构团队或广告商（取决于service level agreement的条款）将需要监测每个营销活动，并根据需要更改相关组件和设置以实现广告商的目标。
...
1. 对于每个广告网络帐户：
   ...
   1. （[您可以在Search、Social和Commerce中创建/编辑](/help/search-social-commerce/introduction/supported-inventory.md)的广告类型）如果该帐户尚未包含促销活动数据，则可以使用以下适用于该广告类型的任意方法，从Search、Social和Commerce中创建促销活动结构和促销活动组件：
      ...
      * （仅限百度、Google Ads、[!DNL LY Ads]、Microsoft Advertising和Yandex帐户）上传[批量工作表文件](/help/search-social-commerce/campaign-management/bulksheets/bulksheet-about.md)，这些文件包含您希望帐户处理的数据量，然后将它们发布到广告网络。

      * （仅限百度、Google Ads、[!DNL LY Ads]、Microsoft Advertising和Yandex帐户）将各个组件的数据直接输入到界面中。 对于大多数促销活动和广告类型，您可以在促销活动级别、广告组级别以及单个关键字、投放位置和广告级别创建数据。
