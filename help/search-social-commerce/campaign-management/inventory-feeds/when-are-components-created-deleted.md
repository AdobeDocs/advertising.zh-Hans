...
| 馈送数据包含在广告中使用的列的新值。 | 上一个文件：广告包含的价格=20<br><br>新文件：对于同一广告，价格=10 | 更改[!DNL LY Ads]、[!DNL Microsoft Advertising]扩展文本广告或[!DNL Yandex]广告的广告副本时，将删除现有广告并创建一个新广告。<br><br>当为其他广告类型更改广告副本或将适用的列用于广告中的[!DNL Google Ads]广告参数（{param1}或{param2}）时，则会更新现有广告。 |
...
| 自上次传播以来，广告的模板设置发生了更改。 | 以前的设置：广告描述=&quot;立即购买[类别]。&quot;<br><br>新设置：广告描述=&quot;立即购买[品牌]。&quot; | 更改[!DNL LY Ads]、[!DNL Microsoft Advertising]扩展文本广告或[!DNL Yandex]广告的广告副本时，将删除现有广告并创建一个新广告。<br><br>如果更改了其他广告类型的广告副本，或者更改反映了广告中用于单个[!DNL Google Ads]广告参数（{param1}或{param2}）的列的更改，则会更新现有广告。 |
...
| 您已对“[!UICONTROL Delete negative keywords when omitted from list]”禁用了营销活动或广告组级别选项。 | 负面关键字列表包括“coupe”和“sport car”。<br><br>广告组已经包括负面关键字“SUV”。 | 任何不在列表中的现有负关键字将保持原样。 |
| 您已为“[!UICONTROL Delete negative keywords when omitted from list]”启用了营销活动或广告组级别的选项，并且存在不在列表中的负关键字。 | 负面关键字列表包括“coupe”和“sport car”。<br><br>广告组已经包括负面关键字“SUV”。 | 通过模板传播馈送文件时，会删除之前使用该模板创建的任何未指定的负关键字。 但是，使用其他方式创建的任何未指定的负面关键字（例如在纯批量处理工作表、[!UICONTROL Campaigns]视图或广告网络的广告编辑器中）将保持不变。 |
