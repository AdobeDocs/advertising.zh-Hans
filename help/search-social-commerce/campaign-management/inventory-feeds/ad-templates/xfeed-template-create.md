...
您可以创建特定于搜索引擎的广告模板，并通过这些模板处理库存数据。 为文本和扩展/扩展文本广告、响应式搜索广告创建单独的模板， [!DNL Google Ads] 购物广告，以及 [!DNL Microsoft Advertising] 购物广告。

除了从头开始创建模板之外，您还可以通过克隆现有模板来创建新模板。

创建或克隆模板并将其与数据馈送文件关联后，您可以通过模板传播文件中的数据，以创建营销活动数据和帐户结构。
