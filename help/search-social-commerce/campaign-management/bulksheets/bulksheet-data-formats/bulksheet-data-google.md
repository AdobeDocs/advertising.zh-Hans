...
| [!UICONTROL DSA Domain Name] | <p>（仅限搜索网络；仅适用于扩展的动态搜索广告）广告网络用于定位动态搜索广告的广告内容的网站的根域（如example.com）或子域（如shoes.example.com）。<br><br><b>注意：</b></p><ul><li><p>扩展的动态搜索广告以网站内容为目标，而不是以关键字为目标。</p></li><li><p>域必须按广告网络的自然搜索索引编制索引才能定位。</p></li><li><p>如果不指定域，则必须创建动态搜索目标，这些目标针对每个广告组的所有网站页面或其子集。</p></li></ul> |
...
| [!UICONTROL Display URL] | 广告中包含的URL。<br><br>对于扩展的动态搜索广告，[!DNL Google Ads]从网站域动态生成此值，您无需输入值。<br><br>对于响应式搜索广告，无需输入值。 显示URL将自动从最终URL中的域提取。 您可以选择使用“路径1”和“路径2”字段自定义URL。<br><br>您无法创建或编辑，但可以删除扩展的文本广告，该广告已于2022年6月弃用。<br><br><b>注意：</b> （具有最终URL的帐户）显示URL和最终URL中的域名必须匹配。[!DNL Google Ads]</p> |
...
| [!UICONTROL Display Path 2] | 第二个显示路径；查看[!UICONTROL Display Path 1]对应的条目。<br><br>示例：如果[!UICONTROL Display Path 1]为“deals”，[!UICONTROL Display Path 2]为“local”，则显示URL为&lt;<i>显示URL</i>>/deals/local，如www.example.com/deals/local。</p><p>您不能创建或编辑，但可以删除扩展的文本广告，其[!DNL Google Ads]已于2022年6月弃用。</p> |
...
| [!UICONTROL Mobile App Platform (Google Adwords)] | （仅限现有应用安装广告）运行移动应用的操作系统： <i>™</i>或<i>ios</i>。 |
...
| [!UICONTROL Ad ID] | <p>标识现有广告的唯一ID。 在CSV和TSV文件中，它前面必须加有一个单引号(&#39;)。[^1]对于响应式搜索广告，编辑或删除广告数据需要广告ID或AMO ID。 对于所有其他实体类型，只有在更改广告状态时才需要广告ID，除非该行包含a)足够的广告属性列来标识广告或b)“[!UICONTROL AMO ID]”。 但是，如果您既不包括[!UICONTROL Ad ID]也不包括[!UICONTROL AMO ID]，并且广告属性列与多个广告匹配，则只有其中一个广告的状态会更改。</p><p><b>注意：</b>如果您编辑a)广告属性列（现有广告的状态除外）或b)响应式搜索广告的任何数据，并且既不包括[!UICONTROL Ad ID]也不包括[!UICONTROL AMO ID]，则会创建新广告，并且现有广告不会发生更改。</p> |
...
| [!UICONTROL RLSA Target ID] | 标识现有营销活动或广告组级别RLSA目标或排除项的唯一ID。 在CSV和TSV文件中，它前面必须加一个单引号(&#39;)。[^1]仅当更改或删除目标或排除项时才需要，除非行中包含目标的“[!UICONTROL AMO ID]”。 |
| [!UICONTROL Device Target ID] | <p>标识现有营销活动级别或广告组级别设备目标或排除项的唯一ID。 在CSV和TSV文件中，它前面必须加一个单引号(&#39;)。[^1]只有在更改或删除目标时才需要，除非该行包含目标的“[!UICONTROL AMO ID]”。</p> |
...
| [!UICONTROL Campaign ID] | 可选 |
| [!UICONTROL Ad Group ID] | 可选 |
| [!UICONTROL Sitelink ID] | 仅在更改或删除站点链接时需要，除非该行包含a)足够的属性列以标识站点链接或b) &quot;[!UICONTROL AMO ID]&quot;。 但是，如果您既不包括[!UICONTROL Sitelink ID]也不包括[!UICONTROL AMO ID]，并且属性列与多个站点链接匹配，则仅其中一个站点链接的状态会更改。<br><br><b>注意：</b>如果您编辑现有站点链接的sitelink属性列（[!UICONTROL Status]除外），并且既不包括[!UICONTROL Sitelink ID]也不包括[!UICONTROL AMO ID]，则会创建一个新的sitelink，并且现有的站点链接不会更改。 |
| [!UICONTROL AMO ID] | 需要编辑或删除数据，除非您包含实体ID和父实体ID。<br><br>Search、Social和Commerce使用该值确定要编辑的正确身份，但不会将ID发布到广告网络。 |
...
| [!UICONTROL Campaign ID] | 可选 |
| [!UICONTROL AMO ID] | 需要编辑或删除数据，除非您包含[!UICONTROL Campaign ID]。<br><br>Search、Social和Commerce使用该值确定要编辑的正确身份，但不会将ID发布到广告网络。 |
