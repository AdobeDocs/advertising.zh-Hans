...
| Adobe记录竞价调整 | （出于提供信息的目的，包含在生成的批量工作表中）Adobe针对促销活动级别位置目标或RLSA建议的只读竞价调整。 仅当营销活动位于具有加权收入目标（而不是“最大化点击量”目标）的项目组合中，并且营销活动包含至少两个位置目标或在过去90天内具有至少五次点击或成本为5美元的RLSA时，才会计算此值。</p><p>如果要手动编辑位置目标或RLSA以使用推荐值，请在创建位置目标或RLSA后至少等待两周，以允许充分的数据收集，并且不要每周多次更改该值。 |
| 设备目标 | <p>（仅限旧版促销活动类型）显示广告的设备： <span style="font-style: italic;"><i>全部</i></span>， <span style="font-style: italic;"><i>计算机</i></span>， <span style="font-style: italic;"><i>智能手机</i></span>，或 <span style="font-style: italic;"><i>平板电脑</i></span>. 对于新营销活动，默认为 <span style="font-style: italic;"><i>全部</i></span>.</p> |
| 设备操作系统目标(Google Adwords) | （仅限旧版促销活动类型；当设备目标包括“智能手机”或“平板电脑”时适用）显示广告的操作系统： <span style="font-style: italic;"><i>全部</i></span>， <span style="font-style: italic;"><i>Android</i></span>， <span style="font-style: italic;"><i>iOS</i></span>，或 <span style="font-style: italic;"><i>掌心</i></span>. 对于新营销活动，默认为 <span style="font-style: italic;"><i>全部</i></span>.</p> |
| 移动运营商(Google Adwords) | <p>（仅限旧版促销活动类型；当设备目标包括“全部”或“智能手机”时适用）智能手机可能连接的移动运营商： <span style="font-style: italic;"><i>全部</i></span>，或者由指示的一个或多个运营商 &lt;c span=&quot;&quot; id=&quot;2&quot; translate=&quot;no&quot; />运营商代码</i></span>>，&lt;<span style="font-style: italic;"><i>国家/地区代码</i></span>>（例如T-Mobile，US）使用 <a href="https://developers.google.com/adwords/api/docs/appendix/codes-formats?csw=1#mobile-carriers" target="_blank">Google Ads的可用运营商和代码</a>. <span style="font-style: italic;"><i>使用分号（例如T-Mobile、US；T-Mobile、GB）分隔多个运营商。 对于新营销活动，默认为 <span style="font-style: italic;"><i>全部</i></span>.</p> |
| 广告组名称 | <p>标识广告组的唯一名称。 最大长度为255个字符；不保存结尾的空白字符（例如，“广告组1”另存为“广告组1”）。 此字段不适用于营销活动级别的站点链接或营销活动级别的设备目标。</p> |
| 广告组类型 | <p>广告组类型： <span class="Option">发现</span> （仅适用于发现活动）， <span class="Option">显示</span> （用于显示促销活动）， <span class="Option">搜索动态</span> （对于扩展的动态搜索广告）， <span class="Option">搜索标准</span> （对于搜索广告）， <span class="Option">购物产品</span> （对于购物产品广告）， <span class="Option">购物展示</span> （不支持创建和编辑），或 <span class="Option">未知</span>.</p> |
| 最大CPC | <p>（仅限CPC促销活动）最大每次点击成本(CPC)，这是在广告网络上为广告点击支付的最高金额，无论是否带有货币符号和标点。 您可以为广告组和关键字、产品组以及动态搜索目标设置值。 新关键字的默认值继承自广告组级别。 对于产品组，您可以设置最低产品组层的值；新层的默认值继承自父层。</p><p>对于优化项目组合中的现有产品组和动态搜索目标，更新仅在一天内有效，并在下一个优化周期中被覆盖。</p><p><b>注意：</b> 对于版面，使用最大版面CPC字段。</p> |
| 最大内容CPC | <p>（仅限CPC营销活动）每次点击的最大内容成本(CPC)，这是为来自显示网络站点的广告点击支付的最高金额，无论是否带有货币符号和标点。 您可以在不以投放位置为目标的营销活动中，在广告组级别设置和编辑值。</p> |
| 受众定位方法 | <p>（仅搜索网络上的营销活动，以及显示网络上现有的只读Gmail营销活动）是否：</p><ul><li><p><span style="font-style: italic;"><i>目标和竞价</i></span>：仅向与目标受众关联、且满足广告组任何其他目标的用户显示广告。</p></li><li><p><span style="font-style: italic;"><i>仅竞价</i></span>：即使向不与目标受众关联的用户显示广告，只要他们满足其他广告组级别的目标即可。</p><p>但是，您可以通过为特定受众设置更高的竞价来增加向这些受众显示广告的几率。</p></li></ul> |
| 关键词 | 关键词字符串。 最大长度为80个字符，不超过10个单词，并且只能包含字母、数字和以下特殊字符：空格 `# $ & _ - " [ ] ' + . / :`</p><p><b>注意：</b></p><ul><li><p>要在广告组或营销策划级别排除关键词，请将“匹配类型”设置为 <span style="font-style: italic;"><i>负面</i></span>. 如果行包含广告组名称，则该广告组将排除关键字。 如果行不包含广告组名称，则该关键字将排除在整个营销活动中。</p></li><li><p>更改Google Ads关键字或匹配类型会删除现有关键字并创建新关键字。</p></li></ul> |
| 投放位置 | （仅使用匹配内容的促销活动）显示网络中可能显示广告的版面。 指定以下选项之一：</p><ul><li class="p"><p>网站：输入有效的URL。 它可以是顶级域、第一级子域或具有单个目录名的域。 URL不能包含问号(？)。 示例：<span class="Code"><br />www.example.com<br />example.com<br />autos.example.com<br />example.com/widgets</span></p></li><li class="p"><p>特定页面上的广告位置：使用格式 `<URL> >> <location,sublocation>` (例如 `finance.google.com >> Company pages,Top right`)。</p></li><li class="p"><p>主题类别：使用语法 `<category::<category> > <subcategory>` 等等(例如 `category::Industries > Energy & Utilities > Oil & Gas`)。</p></li></ul><p><b>注意：</b> 要在广告组或营销活动级别排除投放位置，请将“匹配类型”设置为 <span style="font-style: italic;"><i>负面</i></span>. 如果行包含广告组名称，则该广告组的版面将被排除。 如果行不包括广告组名称，则该投放位置将在整个营销活动中排除。</p> |
| 自动定位表达式 | <p>（未启用“使用我的网站内容定位我的广告”营销活动设置时必需；否则可选）广告组的动态搜索目标。</p><p>对于所有目标，请使用星号(*)。</p><p>要定位最多三个动态搜索标准，请使用格式 `<category>=<target>`，其中 `<category>` 可以包括以下任意类别。 使用“\[blank space\]和\[blank space\]”为单个类别连接多个目标，使用“”连接多个类别[空格] 和 [空格]“。</p><ul><li class="p"><p><span style="font-style: italic;"><i>类别</i></span>：为具有特定Google内容类别的索引页面显示扩展的动态搜索广告。</p></li><li class="p"><p><span style="font-style: italic;"><i>URL</i></span>：为具有特定URL的索引页面显示扩展的动态搜索广告，其中值可能包含在URL中的任意位置。</p></li><li class="p"><p><span style="font-style: italic;"><i>页面标题</i></span>：为具有页面标题中特定文本的索引页面显示扩展的动态搜索广告。</p></li><li class="p"><p><span style="font-style: italic;"><i>页面内容</i></span>：为具有特定内容的索引页面显示扩展的动态搜索广告。</p></li></ul><p>示例： url=shoes.example.com和page title=footwear</p> |
| 父产品分组 | 任何父产品组的层次结构。<br><br>示例： `All Products>>ProductTypeL1=a>>ProductTypeL2=b` |
| 产品分组 | <p>产品组（如“brand=acme”或“所有产品”）。</p><p><b>注意：</b></p><ul><li><p>当指定的产品组在父产品组层次结构中不存在时，Search、Social和Commerce会创建所需的层次结构的任何部分。</p></li><li><p>每当您在Google购物营销活动中通过默认竞价设置为广告组默认竞价创建广告组时，“搜索、Social &amp; Commerce”都会自动创建“所有产品”组。 Search、Social和Commerce会在产品组层次结构的每个级别自动创建一个包含广告组默认竞价的“其他所有项”组。 您仍然可以显式创建这些默认组，并排除它们或更改其竞价。</p></li><li><p>每个广告组最多可包含八层产品组，包括“所有产品”和七个其他层。</p></li></ul> |
| 分区类型 | 产品组的分区类型： <i>细分</i> （当具有子产品组时）或 <i>单位</i> （当它没有子产品组时）。 |
...
| 创意首选设备 | （文本广告、扩展的动态搜索广告和增强型站点链接；可选）您喜欢显示广告的设备类型： <span style="font-style: italic;"><i>全部</i></span> （默认）或 <i>移动设备</i>. 时间 <i>移动设备</i> 指定，则网络将尝试向移动设备用户显示广告，而不是桌面或平板电脑用户。 否则，网络会在任何设备类型上显示广告。</p><p><b>注意：</b></p><ul><li><p>仅管理员和 [!DNL Adobe] 帐户管理员用户可以编辑此设置。</p></li><li><p>网络不保证会在首选设备类型上显示广告。</p></li><li><p>新的增强型站点链接只能在具有现有增强型站点链接或没有站点链接的活动中创建。</p></li></ul> |
| 广告标题，广告标题2-15 | （仅限扩展文本广告和响应式搜索广告）广告的头条，每个标题用竖线分隔( | )。 每个广告标题字段的最大长度为30个字符或15个双字节字符，包括任何动态文本（例如关键字和广告自定义器的值）。</p><p>对于响应式搜索广告，“广告标题”、“广告标题2”和“广告标题3”是必填字段，所有其他广告标题字段都是可选字段。 要删除非必填字段的现有值，请使用值 [delete]</code> （包括括号）。</p><p>对于响应式搜索广告，请使用以下格式插入广告自定义程序：</p><ul><li><p>Google广告： `{CUSTOMIZER.AdCustomizerName:DefaultText}`，例如 `{CUSTOMIZER.Discount:10%}`</p></li><li><p><span>Microsoft Advertising： `{CUSTOMIZER.Attribute name:default text}`，例如 `{CUSTOMIZER.Discount:10%}`</span></p></li></ul><p>您不能创建或编辑，但可以删除扩展的文字广告，Google广告已于2022年6月弃用。 |
| 广告标题1-15位置 | <p>（仅限响应式搜索广告；可选）固定相应广告标题的位置： `[null]` （无值，这使得广告标题适用于所有职位）， <i>1</i>， <i>2</i>，或 <i>3</i>. 例如，如果“广告标题位置”的值为1，则“广告标题”将仅显示在“位置1”中。 默认情况下，所有广告标题为空（没有值）。</p><p>要删除现有值，请使用值 [delete]</code> （包括括号）。</p><p><b>注意：</b> 您可以将多个广告标题固定到同一位置。 广告网络将使用固定到该位置的广告标题之一。 固定到位置3的标题不能与广告一起显示。</p> |
| 描述行1-4 | <p>（仅限扩展的动态搜索广告、扩展文本广告和响应式搜索广告）广告的正文。 每个描述字段的最大长度为90个字符或45个双字节字符，包括任何动态文本（例如关键字和广告自定义器的值）。</p><p>对于响应式搜索广告，请使用以下格式插入广告自定义程序：</p><ul><li><p>Google广告： `{CUSTOMIZER.AdCustomizerName:DefaultText}`，例如 `{CUSTOMIZER.Discount:10%}`</p></li><li><p><span>Microsoft Advertising： `{CUSTOMIZER.Attribute name:default text}`，例如 `{CUSTOMIZER.Discount:10%}`</span></p></li></ul><p>对于展开的动态搜索广告，请仅使用“描述行1”和“描述行2”。 <b>注意：</b> 对于此广告类型，更改广告副本会删除现有广告并创建新广告。</p><p>您不能创建或编辑，但可以删除扩展的文字广告，Google广告已于2022年6月弃用。</p><p>对于响应式搜索广告，“说明行1”和“说明行2”为必需字段，“说明行3”和“说明行4”为可选字段。 要删除现有值，请使用值 [delete]</code> （包括括号）。</p> |
| 描述行1-4位置 | （仅限响应式搜索广告；可选）固定相应描述的位置： `[null]` （无值，因此说明适用于所有职位）， <i>1</i>， <i>2</i>，或 <i>3</i>. 例如，如果“描述1位置”的值为1，则“描述1”将仅显示在“位置1”中。 默认情况下，不会将任何描述固定到位置。</p><p>要删除现有值，请使用值 `[delete]` （包括括号）。</p><p><b>注意：</b> 可以将多个描述固定到同一位置。 广告网络将使用固定到该位置的描述之一。 固定到位置2的描述可能无法与广告一起显示。 |
| 显示URL | 广告中包含的URL。<br><br>对于扩展的动态搜索广告，Google Ads会从网站域动态生成此值，您无需输入值。<br><br>对于响应式搜索广告，您无需输入值。 显示URL将自动从最终URL中的域提取。 您可以选择使用“路径1”和“路径2”字段自定义URL。<br><br>您不能创建或编辑，但可以删除扩展的文字广告，Google广告已于2022年6月弃用。<br><br><b>注意：</b> （具有最终URL的帐户）显示URL和最终URL中的域名必须匹配。</p> |
...
| 显示路径2 | 附加显示路径；请参阅“显示路径1”条目。<br><br>示例：如果显示路径1为“deals”，而显示路径2为“local”，则显示URL为&lt;<i>显示URL</i>>/deals/local，例如www.example.com/deals/local。</p><p>您不能创建或编辑，但可以删除扩展的文字广告，Google广告已于2022年6月弃用。</p> |
| 促销行 | (仅限Google产品列表广告)要包含在搜索结果中的产品列表内的可选促销行。 最大长度为45个字符。</p><p>根据广告在页面上的显示位置，促销行可能会出现在与广告相关的不同位置（例如广告下方）。 |
...
| 排除平板电脑(Google Adwords) | （仅限现有应用程序安装广告）</p><p>（可选）阻止Google Ads向平板电脑用户显示广告。 值可以包括 <i>是</i> 和 <i>否</i>. |
| 登陆页面后缀 | 要附加到最终URL末尾以跟踪信息的任何参数。 示例： `param2=value1&param3=value2`<br><br>参见“[的点击跟踪格式 [!DNL Google Ads]](/help/search-social-commerce/tracking/formats-click-tracking-google.md).”<br><br>较低级别的最终URL后缀会覆盖帐户级别的后缀。 为便于维护，除非需要对各个帐户组件进行不同的跟踪，否则请仅使用帐户级别的后缀。 要在广告组级别或更低级别配置后缀，请使用Google广告编辑器。 |
| 跟踪模板 | 跟踪模板，用于指定所有离场域重定向和跟踪参数，并将最终URL嵌入到ValueTrack参数中。 最粒度级别的跟踪模板（使用关键字作为最粒度）将覆盖所有更高级别的值。<br><br>对于Adobe广告转化跟踪（在营销活动设置包括“EF重定向”和“自动上传”时应用），Search、Social和Commerce会在您保存记录时自动附加其自身的重定向和跟踪代码。<br><br>对于第三方重定向和跟踪，请输入一个值。 有关在跟踪模板中指示最终URL的ValueTrack参数列表，请参阅 <a href="https://support.google.com/google-ads/answer/2375447?hl=en" target="_blank">Google Ads文档</a>.<br><br>要删除现有值，请使用值 `[delete]` （包括括号）。 |
...
| Audience | 搜索广告(RLSA)的目标受众或营销活动或广告组的已排除受众的再营销列表。 您将在“目标类型”字段中指定它是目标还是排除项。 |
...
| RLSA目标状态 | 促销活动或广告组级别RLSA目标或(仅限Google广告)排除的状态： <span style="font-style: italic;"><i>活动或删除</i></span> （仅限现有目标）。 新目标或排除项的默认值是 <span style="font-style: italic;"><i>活动</i></span>. 要删除活动目标或排除项，请输入值 <span style="font-style: italic;"><i>已删除</i></span>. |
...
| 广告ID | <p>标识现有广告的唯一ID。 在CSV和TSV文件中，它的前面必须带有单引号(&#39;)。[^1] 对于响应式搜索广告，编辑或删除广告数据需要广告ID或AMO ID。 对于所有其他实体类型，只有在更改广告状态时才需要AMO ID，除非该行包含a)足以识别广告的广告属性列或b)“AMO ID”。 但是，如果您既不包括广告ID也不包括AMO ID，并且广告属性列与多个广告匹配，则仅其中一个广告的状态将更改。</p><p><b>注意：</b> 如果您编辑a)广告属性列（现有广告的状态除外）或b)响应式搜索广告的任何数据，并且既未包含广告ID也未包含AMO ID，则会创建新广告，并且现有广告不会发生更改。</p> |
...
| 站点链接ID | 标识现有站点链接的唯一ID。 在CSV和TSV文件中，它的前面必须带有单引号(&#39;)。[^1] 仅当更改或删除站点链接时才需要，除非该行包含a)足够的属性列来标识站点链接或b)“AMO ID”。 但是，如果您既不包括站点链接广告ID也不包括AMO ID，并且属性列与多个站点链接匹配，则只有其中一个站点链接的状态将更改。</p><p><b>注意：</b> 如果您编辑现有站点链接的“状态”之外的站点链接属性列，并且既不包括站点链接ID也不包括AMO ID，则会创建新的站点链接，并且现有的站点链接不会发生更改。 |
| RLSA目标ID | 标识现有营销活动或广告组级别RLSA目标或(仅限Google广告)排除项的唯一ID。 在CSV和TSV文件中，它的前面必须带有单引号(&#39;)。[^1] 仅当更改或删除目标或排除项时才需要，除非行包含目标的“AMO ID”。 |
...
| 劐免请求 | （出于提供信息的目的，包含在生成的批量工作表中）用于显示广告违反的任何Google广告策略的名称和文本的占位符。 |
