...
| 列 | 描述 |
| ---- | ---- |
| \[特定于广告商的自定义（派生）量度\] | 的值 [您已创建的自定义量度](/help/search-social-commerce/common-tasks/custom-metrics/custom-metric-about.md) 从现有量度计算得出。 |
| \[广告商特定的标签分类\] | 当前在实体级别应用于实体的任何标签分类。 多个标签分类使用逗号(，)分隔。 |
| \[广告商特定的事务属性\] | 指定事务属性或网站参与度量的转化次数。 |
| \[Google跟踪的转化/事务属性\] | 请参见“GGL\*、GGL_CT\*和GGL_XD_CT\*条目。” |
| [!UICONTROL 7-Day Click Accuracy] | ([!UICONTROL Portfolio Report])过去七天（不包括当天，不包括报表指定的日期范围）的点击预测的平均准确性，以百分比表示。 |
| [!UICONTROL 7-Day Cost Accuracy] | ([!UICONTROL Portfolio Report])以百分比表示的前七天成本预测的平均精确度，不包括当天（不包括报表指定的日期范围）。 |
| [!UICONTROL 7-Day Revenue Accuracy] | ([!UICONTROL Portfolio Report])过去七天（不包括当天，不包括报表指定的日期范围）的收入预测的平均准确性，以百分比表示。 |
| [!UICONTROL Account] | 帐户名称。 |
| [!UICONTROL Account Status] | 搜索、社交和商务中帐户的状态： <ul><li><i>[!UICONTROL Enabled]：</i> （默认）对于同步的广告网络，搜索、社交和商务可以登录到广告网络帐户以检索促销活动数据，并且还启用了其他适用功能，例如优化和跟踪生成。<br><br>对于未同步的广告网络，可以使用优化和/或跟踪生成等适用功能。</li><li><i>[!UICONTROL Disabled]：</i> 对于同步的广告网络，搜索、社交和商务不会登录到广告网络帐户，因此不会检索促销活动数据，并且其他适用的功能（例如优化和跟踪生成）将被禁用。 在启用帐户时收集的数据仍会存储，但您将来创建的所有营销活动管理视图和所有报表都不会包含禁用帐户时段的数据。<br><br>对于未同步的广告网络，不提供适用的功能，例如优化和/或跟踪生成。</li></ul> |
...
| [!UICONTROL Adgroup MBA] | ([!DNL Google Ads]， [!DNL Microsoft Advertising]、和 [!DNL Yahoo! Japan Ads] campaigns)当前的广告组级别的移动竞价调整，它确定在移动设备上显示广告时如何调整竞价。 |
...
| [!UICONTROL Base URL] | 关键字的基本URL，包括为促销活动或帐户配置的任何附加参数。 它不包括任何搜索、社交和商务重定向及跟踪代码。 |
...
| [!UICONTROL Constraint Type] | ([!UICONTROL Constraint Report])约束类型：  <i>[!UICONTROL Bid/Pos Constraint]</i>， <i>[!UICONTROL Bid Shift]</i>， <i>[!UICONTROL Campaign Budget]</i>， <i>[!UICONTROL Context Sensitive Bid]</i>， <i>[!UICONTROL Incremental Bidding]</i>， <i>[!UICONTROL Max CPA]</i>， <i>[!UICONTROL Min Margin]</i>， <i>[!UICONTROL Variable Bid Position]</i>， <i>[!UICONTROL Search Engine Min Bid]</i>，或 <i>[!UICONTROL Variable Position]</i>. |
| [!UICONTROL Constraints] | 图元上任何适用约束的名称，用逗号分隔。 |
| [!UICONTROL Content IS] | 您收到的展示次数/受众网络上广告的展示次数除以您有资格收到的预计展示次数。 在 [!DNL Microsoft Advertising]，这称为&quot;[!UICONTROL Audience IS]“ |
| [!UICONTROL Content IS Lost (budget)] | 由于每日或每月预算过低，在显示/受众网络上的广告未收到的估计展示次数百分比。 在 [!DNL Microsoft Advertising]，这称为&quot;[!UICONTROL Audience lost IS (budget)]“ |
| [!UICONTROL Content IS Lost (rank)] | 由于广告排名不佳而未显示在显示/受众网络上的广告的预计展示次数百分比。 在 [!DNL Microsoft Advertising]，这称为&quot;[!UICONTROL Audience lost IS (rank)]“ |
| [!UICONTROL Conversion Type] | ([!UICONTROL Transaction Report])转换之前的操作：<ul><li><i>[!UICONTROL Click:]</i> 转换前至少发生一次付费点击。</li><li><i>[!UICONTROL Impression:]</i> 在转化之前未发生付费点击，因此转化是因浏览导致的（没有任何付费点击的展示）。</li></ul> |
...
| [!UICONTROL Creative Base URL] | 广告的基本URL，包括为促销活动或帐户配置的任何附加参数。 它不包括任何搜索、社交和商务重定向及跟踪代码。 |
| [!UICONTROL Creative Destination URL] | 广告的最终URL或目标URL（包括任何跟踪参数）。 |
| [!UICONTROL Creative Name] | ([!DNL Yahoo! Japan] 仅限)广告图像名称。 |
| [!UICONTROL Creative Title], [!UICONTROL Creative Title2] - [!UICONTROL Creative Title3] | 广告的标题或标题。 不同的创意类型具有不同的必需标题行数和可选标题行数。 查看 [!UICONTROL Creative Title4] 和中的更高列 [!DNL Microsoft Advertising] 响应式广告或多媒体广告，包括&quot;[!UICONTROL Creative Titles]”列。 |
| [!UICONTROL Creative Titles] | （仅适用于多媒体和响应式搜索广告）为每个广告的简短标题添加一列(”[!UICONTROL Creative Title]“到”[!UICONTROL Creative Title15]“)。 如果包含此列，则无需包含其他列 [!UICONTROL Creative Title] 列，但编辑 [!UICONTROL Order Results/Limit Rows By] 要排序的部分 [!UICONTROL Creative Titles] 而不是 [!UICONTROL Creative Title]. |
...
| [!UICONTROL Currency] | 适用的货币类型（如“美元”或“英镑”）。<br><br><b>注意：</b> 如果报表包含使用不同货币的帐户的数据，则为任意[!UICONTROL Total]“货币值只是列中所有数字的总和，不考虑货币。 |
| [!UICONTROL Current Bid] | 目标的当前出价。 |
| [!UICONTROL Current First Page Bid] | ([!DNL Google Ads] 仅限促销活动)在以下情况下，当前需要在搜索结果第一页放入广告的预计每次点击成本(CPC)竞价： [!DNL Google] 搜索查询与关键字匹配。<br><br>对于单个关键词和匹配类型组合，此值是该组合当前所需的第一个页面竞价。 在多个营销活动中使用相同的关键字和匹配类型组合时，此值是当前所有实例中所需的最低首页出价。 |
...
| [!UICONTROL Description1] - [!UICONTROL Description4] | 广告正文。 不同的创意类型具有不同的必需和可选描述行数。 查看 [!UICONTROL Description3] 和 [!UICONTROL Description4] 中的列 [!DNL Microsoft Advertising] 响应式广告或多媒体广告，包括&quot;[!UICONTROL Descriptions]”列。 |
| [!UICONTROL Descriptions] | ([!DNL Microsoft Advertising] 响应式广告和多媒体广告)为每个广告的描述行添加一列(&quot;[!UICONTROL Description1]“到”[!UICONTROL Description4]“)。 如果包含此列，则无需包含其他列 [!UICONTROL Description] 列。 |
| [!UICONTROL Device] | ([!DNL Google Ads]， [!DNL Microsoft Advertising]， [!DNL Yahoo! Display Network]， [!DNL Yahoo! Japan Ads]、和 [!DNL Yahoo Native] 活动)显示广告的设备类型： <i>[!UICONTROL Computers]</i>， <i>[!UICONTROL Mobile]</i>， <i>[!UICONTROL Tablets]</i>， <i>[!UICONTROL Other]</i>，或 <i>[!UICONTROL N/A]</i> （无值）。 其他广告网络的行的值不适用。<br><br>在搜索营销活动中，如果关键字、广告和/或广告扩展的跟踪模板或目标URL包含用于按设备跟踪数据的参数(`&ev_dvc={device}&ev_dvm={devicemodel}`)，则转化数据也会包含在每种设备类型的行中。 否则，如果转化数据无法归因于某个设备类型，则会汇总到包含&quot;[!UICONTROL Device]的“ ”值 [!UICONTROL N/A]. |
...
| [!UICONTROL EF ID] | ([!UICONTROL Transaction Report])(使用Adobe Advertising转化跟踪服务和&quot;[!UICONTROL EF Redirect]”带有令牌的跟踪方法)用于点击或转化的令牌。<ul><li>对象 [!DNL Google Ads] 搜索广告，EF ID为 `{gclid}:G:s`，其中包括Google点击ID (GCLID)和网络类型（搜索时为“s”）。</li><li> 对象 [!DNL Microsoft Advertising] 搜索广告，EF ID为 `{msclkid}:G:s`，其中包括Microsoft点击ID (MSCLKID)和网络类型（“s”用于搜索）。</li><li>对于其他广告网络上的搜索广告，EF ID包括冲浪者ID、点击时间和网络类型。</li><li>对于展示广告，EF ID包括冲浪者ID、点击或展示时间以及网络类型。</li></ul> |
| [!UICONTROL EF Pixel Location ID] | ([!UICONTROL Geo Distribution Report]；仅用于搜索、社交和商务)用于地理位置的内部ID，用于标准化数据。 |
...
| [!UICONTROL Estimated Impressions] | （仅限展示型营销活动）Search、Social和Commerce已跟踪的估计广告展示次数。 此值可能不同于 [!UICONTROL Impressions] 列（可用时），该列显示广告网络提供的值。 |
...
| `GGL*`， `GGL_CT*`、和 `GGL_XD_CT*` [[!DNL Google Ads]-tracked转化/事务属性] | ([!DNL Google Ads] 搜索和购物网络上的营销活动) [!DNL Google Ads]-tracked转化，每个转化最多具有三个单独的事务属性：<ul><li>`GGL*`  — （跟踪时）关键字的转化值，以“GGL”前缀开头（如GGL Purchase）。</li><li>`GGL_CT*`  — 以“GGL_CT”前缀（例如GGL_CT_Purchase）开头的转换次数（计数）。</li><li>`GGL_XD_CT*`  — （当可用于转化类型时，当您跟踪这些类型时）跨设备转化的数量（计数），测量依据为 [!DNL Google Ads] 以“GGL_XD_CT_”前缀(例如GGL_XD_CT_Purchase)开头。</li></ul><br>每次转化都按竞价单位和点击日期进行记录；在事件级别不可用。 有关详情 [!DNL Google Ads]-tracked转化，请参阅&quot;[[!DNL Google Ads] 搜索、社交和商务中的转化数据](/help/search-social-commerce/campaign-management/introduction/google-conversion-data.md)“ |
| [!UICONTROL Impr. (Abs. Top) %] | ([!DNL Google Ads] 仅限于)在自然搜索结果上方显示为第一个广告的广告展示次数百分比。 |
| [!UICONTROL Impr. (Top) %] | ([!DNL Google Ads] 仅限在自然搜索结果上方显示的广告展示次数百分比。 |
...
| [!UICONTROL Is_Click_Objectives] | ([!UICONTROL Portfolio Report]) <i>true</i> 当项目组合包含的营销活动具有 [!UICONTROL Maximize Clicks] 竞价策略，以及 <i>false</i> 否则。 |
...
| [!UICONTROL Link Type] | ([!UICONTROL Keyword Report]； [!DNL Google Ads] 和 [!DNL Microsoft Advertising] 仅限促销活动；仅当为报告指定的归因规则为“Last Event”时，数据才可用)当行报告由于单击广告扩展（而不是广告本身）或产品/购物广告而导致的转化时，此列显示被单击的链接的类型和标题：<ul><li>`pla:*`  — 产品广告列为 `pla:<product ID>`，如“pla：8525822”。</li><li>`sl:*`  — 站点链接列为 `sl:<Sitelink text>`，如“sl：See Current Offers”。</li></ul> |
| [!UICONTROL Listing Match Type] | 广告列表的关键字匹配类型， <i>[!UICONTROL Content]</i> 对于以内容为目标的促销活动中的广告，或者 <i>[!UICONTROL Sitecpc]</i> ，以获取针对站点的营销活动中的投放位置。 对象 [!DNL Microsoft Advertising] 关键字，这可以包括多种匹配类型(如&quot;[!UICONTROL Broad]，[!UICONTROL Exact]“)。 |
| [!UICONTROL Location] | （显示营销活动）目标受众位置。 |
| [!UICONTROL Long Creative Title1] - [!UICONTROL Long Creative Title5] | (在已完成的报表行中， [!DNL Microsoft Advertising] 响应式广告和多媒体广告)广告的长标题。 要查看这些列，请包含&quot;[!UICONTROL Long Creative Titles]”列。 |
| [!UICONTROL Long Creative Titles] | (用于 [!DNL Microsoft Advertising] 响应式广告和多媒体广告)为每个广告的长标题添加一列(&quot;[!UICONTROL Long Creative Title1]“到”[!UICONTROL Long Creative Title5]“)。 |
| [!UICONTROL Market Type] | 市场类型：  <i>[!UICONTROL search]</i> 或 <i>[!UICONTROL social]</i> |
| [!UICONTROL Max Spend % Target] | (项目组合中的营销活动，具有 [!UICONTROL ROI]， [!UICONTROL CPT]，或 [!UICONTROL Marginal Cost per Transaction] 支出策略)项目组合的最大每日预算目标。 |
...
| [!UICONTROL Min Spend (%)] | ([!UICONTROL Network Constraint Report])为广告网络配置的投资组合最小支出百分比。 对于使用约束类型&quot;[!UICONTROL Min-Max]，”这是 [!UICONTROL Min %] 值，如果 [!UICONTROL Min %] 已配置。 对于使用约束类型&quot;[!UICONTROL Target Spend]，”这是 [!UICONTROL Target Spend] 值。 |
...
| [!UICONTROL Raw Transaction Data] | ([!UICONTROL Transaction Report]交易财产收入（如登记1项或订购12美元12项）。 如果多个竞价单位具有相同的交易ID，则跟踪ID的收入根据指定点击日期（点击数据可用时）的点击数进行拆分。 |
| [!UICONTROL Reach] | ([!DNL Meta] ads)查看您的广告至少一次的人数。 注意： [!DNL Meta] 每天删除重复的用户配置文件范围，因此报告的数据来自 [!DNL Meta] 而搜索、社交和商业可能有所不同。 |
| [!UICONTROL Region] | ([!UICONTROL Geo Distribution Report]， [!UICONTROL Keyword Report])印象或点击源自的区域或美国/加拿大州。 根据用户的IP地址确定。 |
...
| [!UICONTROL Search impr. share] | ([!DNL Google Ads] （仅限）您收到的展示次数除以您有资格收到的预计展示次数。 不足10%的百分比显示为“&lt;10%”，超过90%的百分比显示为“`>90%`“ |
| [!UICONTROL Search lost abs. top IS (budget)] | ([!DNL Google Ads] 和 [!DNL Microsoft Advertising])由于每日或每月预算太低，您的广告不是高于自然搜索结果的第一次广告的时间百分比。 对于Google广告促销活动，超过90%的百分比表示为“`>90%`”或“`0.9001`“ |
| [!UICONTROL Search lost abs. top IS (rank)] | ([!DNL Google Ads] 和 [!DNL Microsoft Advertising])由于广告排名不佳，您的广告不是排名最前的广告，因而高于自然搜索结果的时间百分比。 对于Google广告促销活动，超过90%的百分比表示为“`>90%`”或“`0.9001`“ |
| [!UICONTROL Search lost IS (budget)] | ([!DNL Google Ads] 仅限于)由于每日或每月预算太低而未显示广告的时间百分比。 此量度仅在营销活动级别可用。 90%以上的百分比表示为“`>90%`”或“`0.9001`“ |
| [!UICONTROL Search lost IS (rank)] | ([!DNL Google Ads] 仅限于)由于广告排名不佳而未显示广告的时间百分比。 90%以上的百分比表示为“`>90%`”或“`0.9001`“ |
| [!UICONTROL Search lost top IS (budget)] | ([!DNL Google Ads] 和 [!DNL Microsoft Advertising])由于每日或每月预算太低而未在免费搜索结果上方显示广告的时间百分比。 对象 [!DNL Google Ads] 促销活动，超过90%的百分比表示为“`>90%`”或“`0.9001`“ |
| [!UICONTROL Search lost top IS (rank)] | ([!DNL Google Ads] 和[！DNL [!DNL Microsoft Advertising]])由于广告排名不佳而使广告未显示在自然搜索结果上方的时间百分比。 对于Google广告促销活动，超过90%的百分比表示为“`>90%`”或“`0.9001`“ |
...
| [!UICONTROL Site] | (域引用报告和 [!UICONTROL Keyword Report]；面向站点的投放)产生点击的站点。 |
| [!UICONTROL Start Date] | 报告的第一天。 |
| [!UICONTROL State] | (地理分布报表， [!UICONTROL Keyword Report])交易起源的国家/地区。 根据用户的IP地址确定。 |
...
| [!UICONTROL Tracking URL] |（仅限以搜索为目标的关键词）跟踪模板或嵌入了（适用时）搜索、社交和商务跟踪代码的目标URL。 | | [!UICONTROL Transaction Property Name] | ([!UICONTROL Transaction Report])该交易贷记到的特定于广告商的交易属性。 | | [!UICONTROL Transaction Time] | ([!UICONTROL Transaction Report])记入指定交易属性的时间。 | | [!UICONTROL User Account Type] |已过时 | | [!UICONTROL User SE Account ID] | Search、Social &amp; Commerce分配给广告网络的数值ID。 | | [!UICONTROL Video Average Play Time] | ([!DNL Meta] ads)单次展示时播放视频的平均时间，包括重新播放视频所花费的时间。 | | [!UICONTROL Video Plays] | ([!DNL Meta] 广告)视频开始播放的次数，不包括重播。 | | [!UICONTROL VideoQuartile25Rate]， [!UICONTROL VideoQuartile50Rate]， [!UICONTROL VideoQuartile75Rate]、和 [!UICONTROL VideoQuartile100Rate] |（视频广告）视频播放完25%、50%、75%或100%的百分比。 | | [!UICONTROL View Rate] |（视频广告）查看次数或参与次数除以广告显示次数（视频和缩略图展示次数）。 | | [!UICONTROL Views] |（视频广告）用户观看或参与您的广告的次数。 | | [!UICONTROL ViewThroughConversions] |（受众网络上的广告）由一个或多个展示次数产生，但没有点击的转化次数。 |
