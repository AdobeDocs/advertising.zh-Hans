...
| [!UICONTROL Account Number] | （[!UICONTROL MSA Ad Extension]个报告）广告网络分配的帐户标识符。 |
| [!UICONTROL Account Status] | （[!UICONTROL MSA Ad Extension]个报告）帐户状态： *[!UICONTROL Active]*、*[!UICONTROL Paused]*&#x200B;或&#x200B;*[!UICONTROL Inactive]*。 |
| [!UICONTROL Ad Distribution] | （[!UICONTROL Bing Ads Geo Report]、[!UICONTROL MSA Network Impression Share Report]和[!UICONTROL MSA Network Performance Report]）广告分发位置： *[!UICONTROL Audience]* （在受众网络上）*、[!UICONTROL Search]* （在搜索结果页面上）或&#x200B;*[!UICONTROL Content]* （在[!DNL Microsoft]拥有的网站或参与合作伙伴网站上）。 |
| [!UICONTROL Ad Extension ID] | （[!UICONTROL MSA Ad Extension]个报告）广告扩展的ID。 |
...
| [!UICONTROL Display Name] | ([!UICONTROL Google Performance Max Placement Report])投放位置中显示的名称，如网站URL、移动设备应用程序名称或[!DNL YouTube]视频的名称。 |
...
| [!UICONTROL Keyword] | 关键字。<br><br><b>注意：</b>如果报告在启用了内容的搜索促销活动中包含来自广告组的数据，则此列包含适用的广告组名称，例如“（广告组内容）您的广告组名称”。 对于搜索营销活动中的网站定向投放，此列没有值。 |
...
| [!UICONTROL Placement] | ([!UICONTROL Google Performance Max Placement Report])投放位置的名称。 |
| [!UICONTROL Placement Type] | ([!UICONTROL Google Performance Max Placement Report])投放广告的投放类型： *[!UICONTROL Mobile App]*、*[!UICONTROL YouTube Video]*（广告出现在[!DNL YouTube]视频中或与其相邻）或&#x200B;*[!UICONTROL Webpage]*。 |
...
| [!UICONTROL Primary Status Reason] | ([!UICONTROL Google Asset Group Performance Report])有关资产组主状态的更多详细信息。 值可能包括&#x200B;*ASSET_GROUP_DISAPPROVED，* *ASSET_GROUP_LIMITED，* *ASSET_GROUP_PAUSED，* *ASSET_GROUP_REMOVED，* *ASSET_GROUP_UNDER_REVIEW，* *CAMPAIGN_ENDED，* *CAMPAIGN_PAUSED，* *CAMPING_PENDING，* *CAMPAIGN_REMOVED，* *未知，*&#x200B;或&#x200B;*未指定。* |
...
| [!UICONTROL Search impr. share] | （仅限[!DNL Google Ads]；[!UICONTROL Campaign Daily Impression Share Report]和[!UICONTROL Keyword Daily Impression Share Report]）您收到的展示次数除以您有资格收到的预计展示次数。 10%以下的百分比表示为“`<10%`”，90%以上的百分比表示为“`>90%`”。 |
...
| [!UICONTROL Search IS% Lost to Rank] | （[!UICONTROL Campaign Daily Impression Share Report]和[!UICONTROL Keyword Daily Impression Share Report]）由于广告排名不足，您的广告在搜索网络上未收到的预计展示次数百分比。 低于10%的百分比表示为“`<10%`”，高于90%的百分比表示为“`>90%`”。<br><br><b>注意：</b> [!DNL Microsoft Advertising]营销活动的数据仅在2019年10月11日之前可用。 |
...
| [!UICONTROL Target URL] | ([!UICONTROL Google Performance Max Placement Report])投放位置的URL，如网站、应用商店中移动设备应用程序的链接，或[!DNL YouTube]视频的URL。 |
